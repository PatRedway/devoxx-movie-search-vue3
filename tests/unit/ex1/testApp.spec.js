...
describe('Testing App component', () => {
  ...
  it('Display Joker movie', async () => {
    const wrapper = await mount(App)

    const expectedTitle = wrapper.find('[data-test="title-movie"]').text()
    expect(expectedTitle).toBe('Joker')
  })

  it('Shows year 2019', async () => {
    const wrapper = await mount(App)

    const expectedYear = wrapper.find('[data-test="year-movie"]').text()
